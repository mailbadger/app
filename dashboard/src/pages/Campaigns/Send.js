            ...
            <form onSubmit={handleSubmit}>
                <Box>
                    ...
                    <Box margin={{ top: "large", bottom: "small" }}>
                        <Text alignSelf="end">
                            Total recipients: {totalSelectedSubs}
                        </Text>
                        <Text alignSelf="end">
                            Send quota:{" "}
                            {quota &&
...
const handleSubmit =
    (id, setSuccess, createNotification) =>
        ...
        const postForm = async () => {
            try {
                ...
                if (values.metadata.length > 0) {
                    params.default_template_data = values.metadata.reduce(
                        (map, meta) => {
                ...
                await axios.post(endpoints.postCampaignsStart, params)
                createNotification(
...
const SendCampaign = ({ match }) => {
    ...
    if (
        ...
        return (
            <Redirect to={`/dashboard/campaigns/${match.params.id}/report`} />
        )
    }
                        ...
                        <Modal
                            ...
                            form={
                                <DeleteCampaign
                                    id={campaign.data.id}
                                    onSuccess={() =>
                                        history.replace("/dashboard/campaigns")
                                    }
                                    hideModal={() => setShowDelete(false)}
                                />
                            }
