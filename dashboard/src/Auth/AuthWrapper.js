...
const AuthWrapper = ({ fetchUser }) => {
  ...
  return (
    <Grid
      ...
      style={{ height: "100vh" }}
    >
      {shouldShowSidebar && <AuthSidebar fill gridArea="image" />}
      <Box
        fill
        gridArea="main"
        direction="row"
      ...
      >
        <Switch>
          <Route
            path="/login"
            component={() => (
              <Login isMobile={isMobile} fetchUser={fetchUser} />
            )}
          />
          <Route
            path="/signup"
            component={() => (
              <Register isMobile={isMobile} fetchUser={fetchUser} />
            )}
          />
          <Route path="/forgot-password/:token" component={NewPassword} />
          <Route
            path="/forgot-password"
            component={() => <ForgotPassword isMobile={isMobile} />}
          />
          <Route
            path="/"
            component={() => (
